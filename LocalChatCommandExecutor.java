...
public class LocalChatCommandExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (sender instanceof Player) {
			...
			if (args.length == 0) {
				if (LocalChat.hasPermissions(player, "LocalChat.status")) {
					if (args.length >= 2) {
				...
				if (args[0].equalsIgnoreCase("help")) {
					if (LocalChat.hasPermissions(player, "LocalChat.help")) {
					...
					if (LocalChat.hasPermissions(player, "LocalChat.list")) {
					...
					if (LocalChat.hasPermissions(player, "LocalChat.join")) {
						if (args.length >= 2) {
					...
					if (LocalChat.hasPermissions(player, "LocalChat.leave")) {
						String name = player.getName();
					...
					if (LocalChat.hasPermissions(player, "LocalChat.admin.create")) {
						if (args.length >= 2) {
							String group = args[1];						
					...
					if (LocalChat.hasPermissions(player, "LocalChat.admin.delete")) {
						if (args.length >= 2) {
							String group = args[1];
					...
					if (LocalChat.hasPermissions(player, "LocalChat.admin.spy")) {	
						String name = player.getName();
					...
					if (LocalChat.hasPermissions(player, "LocalChat.admin.set")) {
						if (args.length >= 3) {
							Player player1 = player.getServer().getPlayer(args[1]);
	...
	private void showHelp(Player player, String label) {
		player.sendMessage(ChatColor.YELLOW + LocalChat.name + " (" + ChatColor.WHITE + LocalChat.version + ")");
...
private void sendChatGroupMessage(Player player, String chatGroup, String[] args) {
	if (!LocalChat.cmdMap.containsKey(player)) {
